...
class TablaTarea{
  ...
  =============================================*/
    ...
    $usuarios = ControladorUsuarios::ctrMostrarUsuarios("id", $tareas[$i]["id_admin"]);
	        ...
	        $descripcion = "<p>".$tipo["tipo"]." de GPS ".$dispositivo["modelo"]." en el vehiculo ".$vehiculo["placa"].",</p> <p>Fecha y Hora de instalacion ".$fecha." ".$tareas[$i]["hora"]."</p>";
			...
			$acciones = "<div class='btn-group'><div class='tooltipb'><button class='btn btn-danger btnCancelarTarea' idTarea='".$tareas[$i]["id"]."'' ><i class='fa fa-times'></i></button><span class='tooltipbtext'>Cancelar Tarea</span></div> <div class='tooltipb'><button class='btn btn-success btnTerminarTarea' idTarea='".$tareas[$i]["id"]."'' ><i class='fa fa-check'></i></button><span class='tooltipbtext'>Marcar Tarea Finalizada</span></div> <div class='tooltipb'> <button idTarea='".$tareas[$i]["id"]."' placa='".$vehiculo["placa"]."' tipoTarea='".$tipo["tipo"]."' dispositivo='".$dispositivo["modelo"]."' fecha='".$tareas[$i]["fecha"]."' class='btn btn-info btnEnviarMensaje'><i class='fa fa-share-square'></i> </button> <span class='tooltipbtext'>Enviar Mensaje</span></div></div>";

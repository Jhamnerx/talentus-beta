...
$(".tablaReportes tbody").on("click", ".btnVerReporte", function(){
	...
	$.ajax({
		...
		success: function(respuesta){
			...
			$("#modalVerReporte .informeReporte").val(respuesta["detalle"]);
...
ELIMINAR REPORTE
=============================================*/
$(".tablaReportes tbody").on("click", ".btnEliminarReporte", function(){

	var idReporte = $(this).attr("idReporte");

  	swal({
    	title: '¿Está seguro de borrar la reporte?',
    	...
    	if(result.value){
      	...
      	window.location = "index.php?ruta=reporte&idReporte="+idReporte;
...
$(".idUsuario1").parent().parent().parent().css("color", "red");
$(".idUsuario1").addClass("cambiarColor");
